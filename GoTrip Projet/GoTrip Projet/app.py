    ...
    if session['choix'] == "pays":
        ...
        if session['choixcircuit'] == "libre":
            ...
            query3 = ''' 
               ...
               '''
            resa_tuple = [('{date}',session['choixcircuit'])]
            
            connection.executemany(query3,resa_tuple)
            connection.commit()
            
            delta = session['datefin'] - session['datedebut']
            nbjours = delta.days
            
            end_date = session['datedebut'] + datetime.timedelta(days=(nbjours//2))
            ...
            if session['choixExistant'] == "Scandinavie (Norvège - Suède)":
                
                circuit_tuple = [("France","Norvège",session['datedebut'],nbjours//2),("Novège","Finlande",end_date,(nbjours-nbjours//2))]
                ...
                numero = requete(connection(), '''SELECT NumResa from Reservation ORDER BY NumResa DESC LIMIT 1 ''')
                num_depla = requete(connection(), '''SELECT Id_Dep from Deplacement ORDER BY Id_Dep DESC LIMIT 2 ''')
                
                query = ''' 
                   INSERT INTO Comporter VALUES (?,?) 
                   '''   
                comporter_tuple = [(numero[0],num_depla[0]),(numero[0],num_depla[1])]
                connection.executemany(query, comporter_tuple)
                connection.commit()
                
            elif session['choixExistant'] == "Benelux (Belgique - Pays-Bas)":
                
                circuit_tuple = [("France","Belgique",session['datedebut'],nbjours//2),("Belgique","Pays-Bas",end_date,(nbjours-nbjours//2))]
            ...
            elif session['choixExistant'] == "Péninsule Ibérique (Espagne - Portugal)":
                
                circuit_tuple = [("France","Espagne",session['datedebut'],nbjours//2),("Espagne","Portugal",end_date,(nbjours-nbjours//2))]
            ...
            elif session['choixExistant'] == "Pays Baltes (Lituanie - Lettonie)":
                
                circuit_tuple = [("France","Lituanie",session['datedebut'],nbjours//2),("Lituanie","Lettonie",end_date,(nbjours-nbjours//2))]
            ...
            elif session['choixExistant'] == "Mer et Montagnes (Croatie - Slovénie)":
                
                circuit_tuple = [("France","Croatie",session['datedebut'],nbjours//2),("Croatie","Slovénie",end_date,(nbjours-nbjours//2))]
                ...
                circuit_tuple = [("France","Pologne",session['datedebut'],nbjours//2),("Pologne","Slovaquie",end_date,(nbjours-nbjours//2))]
